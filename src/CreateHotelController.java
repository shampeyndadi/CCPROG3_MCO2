...
public class CreateHotelController {
    ...
    private CreateHotelView creationView;

    /**
     * Constructor to initialize the Hotels object.
     */
    public CreateHotelController(CreateHotelView creationView){
        this.HotelList = new ArrayList<>(); 
        this.creationView = creationView;

        this.creationView.setAddBtn(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
              
                String hotelName = creationView.getInput();
                int amountOfRooms = creationView.getNumRoomInput();

                if (!doesHotelExist(HotelList, hotelName)){
                    if (amountOfRooms <= 50){
                        HotelList.add(new Hotel(hotelName, amountOfRooms));
                        System.out.println("Hotel added");
                        creationView.getStatusInput(hotelName + " created succesfully!");
                    }else{
                        System.out.println("Error rooms exceeded");
                        creationView.getStatusInput("Maximum of 50 rooms only");
                    }
                }else{
                    System.out.println("Error hotel already exists");
                    creationView.getStatusInput("Creation unsuccessful, hotel already exists");
                }
